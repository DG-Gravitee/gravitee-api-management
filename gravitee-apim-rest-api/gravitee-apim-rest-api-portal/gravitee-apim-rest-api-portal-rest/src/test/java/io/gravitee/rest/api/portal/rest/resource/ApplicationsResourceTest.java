...
public class ApplicationsResourceTest extends AbstractResourceTest {
    ...
    public void shouldGetApplicationsOrderByName() {
        ...
        Mockito.verify(filteringService, times(0)).getApplicationsOrderByNumberOfSubscriptions(anySet(), eq(false), eq(false));
    }
    ...
    public void shouldGetApplicationsOrderByNbSubscriptionsDesc() {
        ...
        doReturn(mockFilteredApp).when(filteringService).getApplicationsOrderByNumberOfSubscriptions(anySet(), isNull(), eq(false));

        final Response response = target().queryParam("order", "-nbSubscriptions").request().get();
        assertEquals(HttpStatusCode.OK_200, response.getStatus());

        Mockito.verify(filteringService).getApplicationsOrderByNumberOfSubscriptions(anySet(), isNull(), eq(false));
    }
