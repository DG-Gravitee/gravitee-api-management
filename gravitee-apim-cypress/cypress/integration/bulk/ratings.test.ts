...
describe('Bulk Rate', () => {
    ...
    it('Should rate the API by API_PUBLISHER', () => {
        gio.management(API_PUBLISHER_USER).apis().getAll().ok().should((response) => {
            let apis = response.body;

            apis.forEach((api, index) => {
                if(index % 2 == 0) {
                    gio.management(LOW_PERMISSION_USER)
                        .apisRating()
                        .addApiRating<ManagementError>(api.id, {
                            rate: ApiFakers.apiRating(),
                        })

                    gio.management(APPLICATION_USER)
                        .apisRating()
                        .addApiRating<ManagementError>(api.id, {
                            rate: ApiFakers.apiRating(),
                        })

                    gio.management(API_PUBLISHER_USER)
                        .apisRating()
                        .addApiRating<ManagementError>(api.id, {
                            rate: ApiFakers.apiRating(),
                        });

                    gio.management(ADMIN_USER)
                        .apisRating()
                        .addApiRating<ManagementError>(api.id, {
                            rate: ApiFakers.apiRating(),
                        });
